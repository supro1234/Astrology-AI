              ...
              <AnimatePresence mode="wait">
                ...
                {activeTab === 'predictions' && (
                  <motion.div key="predictions" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                    {[
                      { tag: t('career') || 'Career',      color: '#3B82F6', icon: '💼', text: formatPrediction('career_prediction') },
                      { tag: t('love') || 'Love',        color: '#EC4899', icon: '❤️', text: formatPrediction('love_prediction') },
                      { tag: t('health') || 'Health',      color: '#10B981', icon: '🌿', text: formatPrediction('health_prediction') },
                      { tag: t('finance') || 'Finance',     color: '#F59E0B', icon: '💰', text: formatPrediction('finance_prediction') },
                      { tag: t('family') || 'Family',      color: '#8B5CF6', icon: '🏠', text: formatPrediction('family_prediction') },
                      { tag: t('spirituality') || 'Spirituality',color: '#06B6D4', icon: '🧘', text: formatPrediction('spirituality_prediction') },
                    ].map((item, i) => (
                      <motion.div
                        key={item.tag}
                        className="glass-card-hover"

...
export default function Setup() {
  ...
  const handleVerify = async () => {
    ...
    if (!openRouterKey.trim()) {
      setError('Please enter your OpenRouter API Key for AI features');
      return;
    }
    setError('');
    setLoading(true);
    ...
    try {
      ...
      login(userId.trim() || 'user', apiKey.trim(), openRouterKey.trim());
    ...
    <div style={{
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          <div>
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.8rem', color: 'var(--text-muted)', marginBottom: '0.4rem', fontWeight: 500 }}>
              OpenRouter API Key <span style={{ color: '#f87171', fontSize: '0.7rem' }}>*</span>
            </label>
            <div style={{ position: 'relative' }}>
              <input
              ...
              <button
                ...
                aria-label={showOrKey ? 'Hide token' : 'Show token'}
                style={{
                  position: 'absolute',
                  right: '0.75rem',
                  top: '50%',
        ...
        <GoldButton
          ...
          loading={loading}
          disabled={!apiKey.trim() || !openRouterKey.trim() || loading}
        ...
        {/* Get trial key */}
        <div style={{ textAlign: 'center', marginBottom: '1.5rem', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
          <div>
            <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
              Don't have an Astrology API key?{' '}
            </span>
            <a
          ...
          <div>
            ...
            <a
              ...
              style={{ color: '#FFD700', fontSize: '0.8rem', fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '0.25rem' }}
            >
              Get OpenRouter Key <ExternalLink size={12} />
            </a>
          </div>
        </div>
